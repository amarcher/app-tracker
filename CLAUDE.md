...
8. **Meta reels** — Instagram and Facebook reel views for each brand in `SOCIAL_ACCOUNTS` (`api/_shared/social-reels.ts`), shown in the private Businesses view. `<metaPrefix>_TOKEN` is exchanged for the brand's Page token; the Instagram professional account is found through the Page's `instagram_business_account`. Instagram reports lifetime `views`/`reach`/`total_interactions` per media (`impressions` and `plays` were retired in Graph v22). Facebook reels come from `/{page}/video_reels` with lifetime `video_insights` (`blue_reels_play_count` as views, `post_impressions_unique` as reach). Metrics are requested one at a time because an unsupported metric fails the whole insights request. The business sync writes a daily reading per reel to `social_reel_snapshots`, which turns lifetime counters into views gained over the selected range.
